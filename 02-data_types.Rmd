...
values to varibles and reserve `=` for passing arguments to functions, for example:
...
R also supports integers. If you find yourself in a situation where you explicitely need an integer
...
There is also `is.numeric()` which tests wether a number is of the `numeric` class:
...
`|` work on vector, doing pairwaise comparisons:
...
because "3" is a character, all the other values get implicitely converted to characters. You have
to be very careful about this, and if you use atomic vectors in your programming, you have to make
absolutely sure that no characters or booleans or whatever else are going to convert your atomic
...
`NaNN`means "Not a Number" and is returned when a function return something that is not a number:
...
but these are in principle only used when you need to program your own functions and need to explicitely test for the missingness of, say,
a character value.
