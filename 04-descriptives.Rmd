...
[link](https://chroniclingamerica.loc.gov/lccn/sn86069133/1910-10-31/ed-1/seq-1/ocr.xml) (to see
how the newspaper looked like, 
[click here](https://chroniclingamerica.loc.gov/lccn/sn86069133/1910-10-31/ed-1/seq-1/)).
...
```
<TextLine HEIGHT="138.0" WIDTH="2424.0" HPOS="4056.0" VPOS="5814.0">
...
We are interested in the strings after `CONTENT=`. In Chapter 10, we are going to explore this file
...
```{r}
winchester <- read_lines("https://chroniclingamerica.loc.gov/lccn/sn86069133/1910-10-31/ed-1/seq-1/ocr.xml")
```
...
So the `winchester` variable si a character atomic vector with 42 elements. So first, we need to
...
Ok, so seems like the first element is part of the header of the file. What about the second one?
...
take quite some time to print, and you will not really be able to make it out anything. The best
...
This returns a boolean atomic vector of the same length of `winchester`. If the string `CONTENT` is
nowhere to be found, the result will equal `FALSE`, if not it will equal `TRUE`. Here it is easy to
see that the last element contains the string `CONTENT`. But what if instead of having 42 elements,
the vector had 24192 elements? And hundreds had the contained the string `CONTENT`? It would be easier
to instead have the indices of the vector where one can find the word `CONTENT`.

This is possible with `str_which()`
...
Here, the result is 42, meaning that the 42nd element of `winchester` contains the string `CONTENT`
...
Now suppose I am interested in philosophers whose name ends in `us`. 


Let us use `str_locate()` first:
...
You can interpret the result as follows: in the rows, this is the index of the vector where the 
...
So now that we know that what interests us in the 42nd element of `winchester`, let's take a closer
look at it:
...
This time, we get the two symbols that immediately precede "us". Instead of continuing to like this
we now use the `*`, which matches zero or more of `.`. So by combining `*` and `.`, we can match 
any symbol repeatetly, until there is nothing more to match. Note that there is also `+`, which works
...
#### Exctrating or removing strings
