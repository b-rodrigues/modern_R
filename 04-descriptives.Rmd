...
need to be their own columns. To do this, we need to use 

```{r}
survey_data %>% 
  pivot_wide(keys = id, names_from = variable, values_from = value)
```

Let's go through `pivot_wide()`'s arguments: the first is `keys = ` which requires the variable
that uniquely identifies the rows. `names_from = ` is where you input the variable that will 
...
```{r}
survey_data %>% 
  pivot_wide(keys = id, names_from = variable, values_from = value, values_fill = list(value = 0))
```

A list of variable and their respective value to replace NA's with must be supplied to `values_fill`. 
...
unemp_lux_data2 %>% 
  pivot_wide(names_from = c(division, year), values_from = unemployment_rate_in_percent)
```
...
I then only kept the columns I'm interested in, pivoted the data to a wide format.
...
unemployment rate for each commune times year. Contrast this to the following example:
...
Going back to the unemployment example, you might have noticed that because there is no data for 
the years 2016 and 2017, these columnsdo do not appear in the data. But suppose that we need to have 
...
the specification directly, just like for the `mtcars` example of before:
