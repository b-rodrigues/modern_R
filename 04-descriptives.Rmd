...
It is also possible to construct a specification data frame, just like for `pivot_wider()`:

```{r}
mtcars_spec_long <- mtcars_wide %>% 
  pivot_longer_spec(matches("0|1"), values_to = "mpg") %>% 
...
```{r}
mtcars_wide %>% 
  pivot_longer(spec = mtcars_spec_long, values_drop_na = TRUE) %>% 
  select(am, cyl, mpg, everything())
```
