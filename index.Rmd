...
This book is still being written. Chapters 1 to 7 are almost ready. Chapter 7 is outdated, but
the key messages are still useful. 10 and 11 are empty for now. Some exercises might be at the 
wrong place too and more are coming.
...
```{r, eval=FALSE}
install.packages(c("tidyverse", "tidymodels", "parsnip", "plm", "pwt9", "checkpoint", "Ecdat",
                "ggthemes", "janitor", "rio", "colourpicker", "glmnet", "mlbench"))
```
...
[Fip](http://www.fipradio.fr/player) or [Vaporwave](https://www.youtube.com/watch?v=u5qwtO_ABPM). 
